...
def del_role(request,del_id):
    obj = models.Role.objects.filter(id=del_id).delete()
    return redirect(reverse('rbac:role_list'))
...
def menu_list(request,):
    ...
    return render(request,'rbac/menu_list.html', {'all_menu':all_menu,
...
def del_menu(request,del_id):
    obj = models.Menu.objects.filter(id=del_id).delete()
    return redirect(reverse('rbac:menu_list'))
...
def permission_menu(request,del_id):
    obj = models.Permission.objects.filter(id=del_id).delete()
    return redirect(reverse('rbac:menu_list'))
