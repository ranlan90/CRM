...
class Customer(models.Model):
    ...
    def show_status(self):
        ...
        return mark_safe('<span style = "background-color: {};color: white;padding:3px" > {} </span>'.format(
            color_dict[self.status], self.get_status_display()))
    ...
    def pay_link(self):
        if not self.paymentrecord_set.exists():
            return mark_safe('<a href="{}">添加缴费表</a>'.format(reverse('add_payment', args=(self.id,))))
        else:
            return mark_safe(
                '<a href="{}">添加</a> | <a href="{}">查看</a>'.format(reverse('add_payment', args=(self.id,)),
                                                                   reverse('payment_record', args=(self.id,))))

    def __str__(self):
        return '{1}<{0}>'.format(self.qq, self.name)
