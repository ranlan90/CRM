...
from crm.forms import RegForm, CustomerForm, ConsultRecordForm, EnrollmentForm, PaymentForm
...
def customer_list(request):
    ...
    return render(request,
                  'crm/consultant/customer_list.html',
...
class CustomerList(View):
    ...
    def get(self, request):
        ...
        query_params = request.GET.copy() #  <QueryDict: {'query': ['alex'], 'page': ['2']}>
        # query_params = copy.deepcopy(request.GET) #  <QueryDict: {'query': ['alex'], 'page': ['2']}>


        # query=alex
        # print(request.GET.urlencode()) # query=alex&page=2
        # query_params['page'] = 1
    ...
    def multi_apply(self):
        ...
        with transaction.atomic():
            ...
            if apply_num == len(obj_list):
                ...
                return HttpResponse('你手速慢了，已经被别人抢走')
    ...
    def get_search_contion(self, fields_list):
        ...
        return q
        # Q(Q('qq__contains', query)) | Q(Q('name__contains', query))
...
class ConsultRecord(View):
    def get(self, request, customer_id):
        ...
        page = Pagination(request, all_consult_record.count(), query_params,)
...
def consult_record(request, edit_id=None):
    ...
    return render(request, 'crm/consultant/edit_consult_record.html',
...
class EnrollmentList(View):
    def get(self, request, customer_id):
        ...
        query_params = request.GET.copy()
        page = Pagination(request, all_record.count(), query_params, )
...
class PaymentList(View):
    def get(self, request, customer_id=None):
        if customer_id == '0':
            all_record = models.PaymentRecord.objects.filter(delete_status=False, consultant=request.user)
        else:
            all_record = models.PaymentRecord.objects.filter(customer_id=customer_id)
        query_params = request.GET.copy()
        page = Pagination(request,all_record.count(),query_params)
        query_params = self.get_query_params()

        return render(request, 'crm/consultant/payment_list.html',
                      {'all_record': all_record[page.start:page.end],
                       'query_params': query_params,
                       'pagination': page.show_li,
                       },
...
# 添加编辑缴费记录
def payment(request, customer_id=None, edit_id=None):
    obj = models.PaymentRecord.objects.filter(id=edit_id).first() or models.PaymentRecord(customer_id=customer_id, consultant=request.user)
    form_obj = PaymentForm(instance=obj)
    if request.method == "POST":
        form_obj = PaymentForm(request.POST, instance=obj)
        if form_obj.is_valid():
            payment_obj = form_obj.save()
            payment_obj.customer.status = 'paid_in_full'
            payment_obj.customer.save()
            next = request.GET.get('next')
            if next:
                return redirect(next)
            else:
                return redirect(reverse('payment_record', args=(0,)))
    return render(request, 'crm/consultant/payment.html', {'form_obj': form_obj})
...
#     if current_page == 1:
#         prev_li = '<li><a class="disabled"><<</a></li>'
#     else:
...
#         next_li = '<li><a class="disabled">>></a></li>'
#     else:
